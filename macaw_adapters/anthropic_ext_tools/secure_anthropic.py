            ...
            if hasattr(response, 'content') and response.content:
                for content_block in response.content:
                    if content_block.type == 'tool_use':
                        ...
                        if func_name not in self.user_tools and mapl_name not in self.user_tools:
                            ...
                            try:
                                ...
                                if 'denied' in error_msg.lower() or 'policy' in error_msg.lower():
                                    logger.warning(f"MACAW blocked tool {mapl_name}: {error_msg}")
                                    result = {
                        ...
                        # Add assistant message with tool use (use original func_name for Claude)
                        messages.append({
                            "role": "assistant",
                            "content": [{"type": "tool_use", "id": content_block.id,
                                        "name": func_name, "input": tool_args}]
                        })
