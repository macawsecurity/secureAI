...
def main():
    ...
    try:
        # Alice authenticates via Entra. Token is minted by get_token.py:
        #   passkey login  -> amr:fido    -> phishing-resistant  (ALLOW path)
        #   password login -> amr:pwd/mfa -> not                 (DENY path)
    ...
    try:
        ...
        alice = MACAWClient(
            ...
            intent_policy={
                ...
                "attestations": [
                    # Precondition: the human must have logged in with a phishing-resistant
                    # method (hardware key / passkey). Deny-if-absent, checked before approval.
                    "phishing_resistant",
