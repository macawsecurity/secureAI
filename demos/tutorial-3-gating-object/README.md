...
A single tool runs SQL across many data products. A rule fixed to the tool cannot say which tables each data product allows, nor hold a query to read-only — that depends on the data it names and on the SQL itself.

So this tutorial attaches the rules to the data. An **object policy** on each data product sets which tables it exposes; a **content verifier** holds every query to read-only, no destructive SQL.

## Attaching a Policy to a Thing

An object policy attaches rules to a data object, not to the tool. Here the object is a **data
product** — a curated set of tables drawn from a data source. The app policy carries the reference
`alation:{params.data_product_id}`, and MACAW resolves it per request:
